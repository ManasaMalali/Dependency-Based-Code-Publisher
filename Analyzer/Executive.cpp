...
int CodeAnalysisExecutive::ExecuteCommandLine(int argc, char* argv[])
{
	std::string cmdstr;
	LPCWSTR text = L"HTMLfiles\\Index.htm";
	std::string cmdstr1 = "HTMLfiles\\Index.htm";
	if (argc == 7)
	{
		cmdstr = argv[6];
		std::string command("start \"\"  \"" + cmdstr + "\"");
		std::system(command.c_str());
		std::string commandNew("start \"\"  \"" + cmdstr1 + "\"");
		std::system(commandNew.c_str());
	}

	else if (argc == 6)
	{
		ShellExecute(0, 0, text, 0, 0, SW_SHOW);
		std::cout << "\n Specify the html filename to display on the web browser" << std::endl;
	}
	return 0;
}

void CodeAnalysisExecutive::ExecuteFunctions(CodeAnalysisExecutive &exec, int argc, char* argv[])
{
...
int main(int argc, char* argv[])
{
	...
	std::cout << "----------------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "REQUIREMENT 10 - Automated unit test suite that demonstrates all the requirements of this project are met. " << std::endl;
	std::cout << "----------------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "REQUIREMENT 9 - Demonstrates the CodePublisher functionality by publishing all the important packages in Project #3." << std::endl;
	std::cout << "--------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "REQUIREMENT 8 - Processing command line arguments, with list of specified file patterns to match. " << std::endl;
	std::cout << "--------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "Below is a list of all the files that is being given : " << std::endl;  
  std::unordered_map<std::string, std::vector<std::pair<int, int>>> MapFiles;
  exec.ExecuteFunctions(exec, argc, argv);
  using namespace CodeAnalysis;   
  ...
  std::cout << "-------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << "REQUIREMENT 7 - Publisher displays the dependencies and link to each of them on the web browser " << std::endl;
  std::cout << "-------------------------------------------------------------------------------------------------" << std::endl;
  std::unordered_map<std::string, std::vector<std::string>> result = Analyzer.Analyze(sourcefiles);  
  for (auto x : result)  {
	  std::cout << " Dependency Analysis " << x.first << ":" << std::endl;
  ...
  std::cout << "-----------------------------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << "REQUIREMENT 3 - Calling Publisher program that provides for creation of web pages each of which captures the content of a single C++ source code file, e.g., *.h or *.cpp. " << std::endl;
  std::cout << "-----------------------------------------------------------------------------------------------------------------------" << std::endl;
  ExpandCollapse ec;
  ...
  p1.HTMLMain();
  std::cout << "-----------------------------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << "REQUIREMENT 4 [BONUS] - Provided the facility to expand or collapse class bodies, methods, and global functions using JavaScript and CSS properties. " << std::endl;
  std::cout << "-----------------------------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << "REQUIREMENT 5 - Provided a CSS style [StyleSheet.css] sheet that the Publisher uses to style its generated pages and a JavaScript file [JavaScript.js] that provides functionality to hide and unhide sections of code for outlining, using mouse clicks. " << std::endl;
  std::cout << "-----------------------------------------------------------------------------------------------------------------------" << std::endl;
  std::cout << "REQUIREMENT 6 - Embed in each web page's <head> section links to the style sheet - \"StyleSheet.css\" and JavaScript file - \"JavaScript.js\"." << std::endl;
  std::cout << "-----------------------------------------------------------------------------------------------------------------------" << std::endl;
  exec.ExecuteCommandLine(argc, argv);  
  return 0;
}